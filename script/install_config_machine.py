...
def install_python():
    ...
    if '2.7' not in PYTHON_VERSION:
        os.system(' echo "[Info] Installation of python ..."')
...
def install_java():
    ...
    if JAVA_VERSION != '1.8.0_112\n':
        os.system(' echo "[Info] Downloading Java ...."')
        os.system('wget -q --no-cookies --no-check-certificate --header "Cookie: oraclelicense=accept-securebackup-cookie" "http://download.oracle.com/otn-pub/java/jdk/8u112-b15/jdk-8u112-linux-x64.tar.gz" -O jdk-8-linux.tar.gz')
        os.system(' echo "[Info] Installation of Spark ..."')
