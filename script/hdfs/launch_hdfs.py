...
def launch():
	...
	run('ssh xnet@' + config.get('NameNode', 'default_standby') + ' \"' + hadoop_dir + '/bin/hdfs namenode -bootstrapStandby\"', shell=True)

	info('Starting standby NN')
	run('ssh xnet@' + config.get('NameNode', 'default_standby') + ' \"' + hadoop_dir + '/sbin/hadoop-daemon.sh start namenode\"', shell=True)
